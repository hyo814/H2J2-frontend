...
import ArmSearch from "./WholeSearch";
...
class WholeList extends Component {
    ...
    loadVideo = async (page) => {
        await axios.get('http://h2j22020.vps.phps.kr:5000/api/exercise/whole_body?page='+page)
            .then(({ data }) => {
                this.setState({
                    loading: true,
    ...
    render() {
        return (
            <>
                <div>
                    <WholePage Exercises={this.state.exercises} stateRefresh={this.stateRefresh} />
                </div>
                <ArmSearch/><CheckCam/>
                <PaginationButton
