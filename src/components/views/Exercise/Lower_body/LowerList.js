...
import ArmSearch from "./LowerSearch";
...
class LowerList extends Component {
    ...
    loadVideo = async (page) => {
        await axios.get('http://h2j22020.vps.phps.kr:5000/api/exercise/lower_body?page='+page)
            .then(({ data }) => {
                this.setState({
                    loading: true,
    ...
    render() {
        return (
            <>
                <div>
                    <LowerPage Exercises={this.state.exercises} stateRefresh={this.stateRefresh} />
                </div>
                <ArmSearch/><CheckCam/>
                <PaginationButton
