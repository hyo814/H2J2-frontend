...
class BackSearch extends Component {
    constructor(props){
        super(props);
        this.state = {
            completed: 0,
    ...
    searchVideo = (title) => {
        axios.get('http://h2j22020.vps.phps.kr:5000/api/exercise/back/search?title='+title)
            .then(({ data }) => {
                this.setState({
                    loading: true,
                    videos: data,
        ...
        return (
            <>
                ...
                <Dialog open={this.state.search_open} onClose={this.handleClose}>
                    <DialogContent>
                        <Backpage Viedos={this.state.videos}/>
                    </DialogContent>
                    <DialogActions>
...
export default withStyles(styles)(BackSearch)
